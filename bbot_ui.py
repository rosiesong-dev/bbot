...
with st.sidebar:
    ...
    if not st.session_state.db_ready:
        ...
        with st.spinner("🔍 DB 확인 중..."):
            ...
            if web_exists:
                print("✅ [1/3] 웹 DB 존재 확인 완료")
            book_exists = table_exists("book_eng")
            if book_exists:
                print("✅ [2/3] 책 DB 존재 확인 완료")
            video_exists = table_exists("video_db")
            if video_exists:
                print("✅ [3/3] 영상 DB 존재 확인 완료")
    ...
    with st.chat_message(msg["role"]):
        ...
        if msg["role"] == "assistant" and "video_source" in msg and msg["video_source"]:
            ...
            if "youtu.be" in video_info["url"] or "youtube.com" in video_info["url"]:
                ...
                embed_url = (
                    f"https://www.youtube.com/embed/{video_id}?start={start_time}"
                )
                ...
                st.markdown(f"**{video_info['title']}**")
                st.markdown(
                    f"[영상 보기]({video_info['url']}) (시작: {int(video_info['start'])}초)"
                )
...
if prompt:
    ...
    if not st.session_state.db_ready:
        st.error(
            "⚠️ DB가 준비되지 않았습니다. 사이드바에서 'DB 생성하기' 버튼을 눌러주세요."
        )
        st.stop()
        ...
        with st.spinner("Searching..."):
            try:
                ...
                if sources_info.get("video_docs"):
                    ...
                    # YouTube 영상 임베드
                    if (
                        "youtu.be" in top_video["url"]
                        or "youtube.com" in top_video["url"]
                    ):
                        # URL에서 비디오 ID 추출
                        ...
                        st.markdown(f"**{top_video['title']}**")
                        st.markdown(
                            f"[영상 보기]({top_video['url']}) (시작: {int(top_video['start'])}초)"
                        )
                ...
                if sources_info.get("web_docs"):
                    web_urls = list(
                        set(
                            [d["url"] for d in sources_info["web_docs"] if d.get("url")]
                        )
                    )
                    if web_urls:
                        sources.append("\n**🌐 웹 자료:**")
                ...
                if sources_info.get("book_docs"):
                    book_names = list(
                        set([d["book"] for d in sources_info["book_docs"]])
                    )
                    if len(book_names) == 1:
                ...
                if sources_info.get("video_docs"):
                    ...
                    for doc in sources_info["video_docs"]:
                        ...
                        sources.append(
                            f"- [{doc['title']}]({doc['url']}) ({video_start}-{video_end})"
                        )
