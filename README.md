...
```bash
conda create -n bbot python=3.12 pip
conda activate bbot
```
...
`.env` 파일을 열어 아래 항목들을 본인 환경에 맞게 수정:
...
#### Option A: Installing using Homebrew (macOS)
...
#### Option B: Installing using Docker
...
`books.zip` 및 `srt_data.zip` 파일을 다운로드 받아 압축 해제
